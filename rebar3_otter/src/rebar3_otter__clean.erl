...
do(State) ->
  lists:foreach(fun clean_app/1, rebar_state:project_apps(State)),
  {ok, State}.
...
clean_app(App) ->
  ...
  case rebar3_otter__config:validate(Raw) of
    {ok, Crates} ->
      ...
      lists:foreach(fun(Crate) -> clean_crate(Crate, AppDir) end, Crates);
    {error, Reason} ->
...
clean_crate(#{name := Name, path := Path}, AppDir) ->
  CratePath = filename:join(AppDir, Path),
  OutFile = filename:join([AppDir, "priv", "native",
